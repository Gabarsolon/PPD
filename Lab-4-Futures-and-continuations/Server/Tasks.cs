...
namespace Server
{
	class Tasks
	{
		private static void HandleUrl(String url)
		{
			...
			Connect(socket, endPoint, new Url(baseUrl, urlPath))
				.ContinueWith((_) => Send(socket, splittedUrl)
				.ContinueWith((_) => Receive(socket, splittedUrl, buffer)
				.ContinueWith((Task<int> bytesRead) =>
				{

					if (bytesRead.Result > 0)
					{
						string responseHeader = HttpParser.ParseHeader(buffer, bytesRead.Result);
